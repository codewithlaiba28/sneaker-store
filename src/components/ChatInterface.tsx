...
import { Send, Loader2, Mic, Square, Sparkles } from 'lucide-react';
...
export default function ChatInterface({ className = '' }: ChatInterfaceProps) {
  ...
  const toggleListen = () => {
    ...
    recognition.onresult = (event: any) => {
      ...
      setTimeout(() => {
        const form = document.getElementById('chat-form-inner') as HTMLFormElement;
        form?.requestSubmit();
      }, 500);
    };
  ...
  return (
    <div className={`bg-[#030303] border border-white/20 rounded-sm shadow-[0_0_50px_rgba(255,255,255,0.05)] overflow-hidden flex flex-col relative group ${className}`}>
      ...
      {/* Corner Flourishes */}
      <div className="absolute top-0 left-0 w-6 h-6 border-t border-l border-white/50 pointer-events-none m-2 z-10" />
      <div className="absolute top-0 right-0 w-6 h-6 border-t border-r border-white/50 pointer-events-none m-2 z-10" />
      ...
      <div className="p-4 bg-[#050505] border-t border-white/20">
        <form id="chat-form-inner" onSubmit={handleSubmit} className="flex gap-3 items-center">
          <div className="relative flex-1">
            <input 
              ...
              className={`w-full h-[50px] bg-[#020202] border ${isListening ? 'border-brand-gold' : 'border-white/20'} text-white pl-5 pr-12 rounded-sm focus:outline-none focus:border-white transition-colors font-light placeholder:text-white/30 text-sm`}
            />
            <button 
              type="button"
          ...
          {isLoading ? (
            <button 
              type="button" 
              onClick={handleStop}
              className="bg-red-900/50 border border-red-500/50 text-white h-[50px] px-6 font-bold tracking-[0.15em] text-xs uppercase hover:bg-red-900 transition-all rounded-sm flex items-center justify-center gap-2 shrink-0"
            >
            ...
            <button 
              type="submit" 
              disabled={!input.trim()}
              className="bg-white text-black h-[50px] px-6 font-bold tracking-[0.15em] text-xs uppercase hover:bg-[#e0e0e0] hover:shadow-[0_0_15px_rgba(255,255,255,0.4)] transition-all disabled:opacity-50 disabled:cursor-not-allowed rounded-sm flex items-center justify-center gap-2 shrink-0"
            >
